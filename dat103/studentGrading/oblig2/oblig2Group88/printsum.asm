...
skrivsiffer:
    ...
    ; Divide the sum by 10 to get the tens place
    mov eax, ecx
    xor edx, edx  ; clear edx for the division
    mov ecx, 10
    div ecx       ; quotient in eax, remainder in edx

    ; Convert the quotient and remainder to ASCII and store them in 'siffer'
    add al, '0'
    add dl, '0'
    mov [siffer], al
    mov [siffer + 1], dl

    ; Prepare for writing the two characters to STDOUT
    mov ecx, siffer
    mov edx, 2
    mov ebx, STDOUT
    mov eax, SYS_WRITE
